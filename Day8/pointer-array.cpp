...
class Skill : public Employee{ // Derived Class(Single Inheritance)
    ...
    public:
    Skill() { //Default Constructor without initilizing any values
        cout << "No Data Found!";
    } 

     Skill (int count){
            skillCount = count;
            skills = new string[skillCount]; // Dynamically Allocating Memory for pointer Array(skill)
       ...
       void showSkills(){
            ...
            for(int i = 0; i < skillCount; i++){ // Display the constent of the Skill array
                cout << i+1 << ". " << skills[i] << endl;
            }
       }

       ~Skill(){
         delete[] skills; //Free up the memory allocated by the pointer Array using delete keyword
